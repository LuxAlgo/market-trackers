...
What this walkthrough does: joins LuxAlgo Market Trackers's congressional-trade disclosures to a price series
...
LuxAlgo Market Trackers ships no price data and computes no scores, signals, or predictions — here or anywhere
...
There are two ways to run this: the copy-paste Python below (using the `market-trackers-data` reader
package from `python/`), or the equivalent one-liner with the LuxAlgo Market Trackers CLI — see
[the CLI equivalent](#the-cli-equivalent) at the end. Both do exactly the same computation
(the CLI's `market-trackers analyze` command is a thin wrapper over
...
```python
subset = trades[trades["ticker"].notna()].copy()          # only rows LuxAlgo Market Trackers resolved to a ticker
...
Disclosed amounts are printed as ranges on the filing (`"$1,001 - $15,000"`) — LuxAlgo Market Trackers never
...
market actually knew, so every price join below (and in `market-trackers analyze`) keys off `filedAt`.

## 3. Bring your own prices

LuxAlgo Market Trackers ships no price data. Supply your own CSV — `date,ticker,close`, one row per
...
- **Coverage gaps are gaps, not absence.** Only rows with a resolved `ticker` are usable here;
  asset descriptions LuxAlgo Market Trackers couldn't resolve to a ticker are excluded from `subset` in step
...
- **No statistical significance testing.** `mean`/`median` are plain descriptive summaries of
  ...
  claim of significance is computed here or anywhere in LuxAlgo Market Trackers.

## The CLI equivalent

The LuxAlgo Market Trackers CLI (TypeScript) does the same join, from a local synced store, with the same
`filedAt`-keyed logic, the same forward-search accommodation, and the same disclaimer on every
result:

```bash
market-trackers analyze congress --prices your_prices.csv --window 30 --json
```
