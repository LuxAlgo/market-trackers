"""Offline tests for market_trackers_data, run with plain `python3 -m unittest`.

Every case reads from python/tests/fixtures/ — no network access, no live
market-trackers-data checkout required.
"""
...
# Make `market_trackers_data` importable without installing the package, regardless
# of the current working directory this test is invoked from.
PACKAGE_ROOT = Path(__file__).resolve().parents[1]  # .../python
if str(PACKAGE_ROOT) not in sys.path:
    sys.path.insert(0, str(PACKAGE_ROOT))

from market_trackers_data import TrackerDataError, load_manifest, load_snapshot, to_dataframe  # noqa: E402
...
class LoadManifestTests(unittest.TestCase):
    ...
    def test_rejects_json_that_is_not_a_manifest(self):
        with self.assertRaises(TrackerDataError) as ctx:
