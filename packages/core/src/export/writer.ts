 ...
 * `market-trackers import` reads them back, the published dumps are the durable
...
export async function exportDumps(
  ...
  for (const dataset of datasets) {
    ...
    // snapshotOnly datasets (wholesale-replaced bulk data, e.g. patents):
    // every ingestion-day export — deltas, latest.json, feed.xml, entity
    // feeds — would be the entire multi-GB history re-emitted per refresh,
    // and the entity-feed pass would materialize all of it in memory. Year
    // shards + the manifest ARE the export for them.
    if (options.deltas !== false && !dataset.snapshotOnly) {
...
export async function buildManifest(
  ...
  for (const d of report.datasets) {
    ...
    datasets[d.dataset] = {
      ...
      feed: d.rowCount > 0 && !def.snapshotOnly ? `${def.exportDir}/feed.xml` : null,
