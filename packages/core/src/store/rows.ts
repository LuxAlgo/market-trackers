...
import type { CongressHearing, CongressHearingCommittee } from "../schema/congress-hearing.js";
...
export const congressHearingMapper: RowMapper<CongressHearing> = {
  toRow(r) {
    return {
      id: r.id,
      ...
      congress: r.congress,
      session: r.session,
      held_date: r.heldDate,
      citation: r.citation,
      committees: toJson(r.committees),
      witnesses: toJson(r.witnesses),
      member_bioguide_ids: toJson(r.memberBioguideIds),
  ...
  fromRow(row) {
    return {
      ...
      docClass: nullableStr(row.doc_class),
      congress: Number(row.congress),
      session: toNullableNumber(row.session),
      heldDate: str(row.held_date),
      citation: nullableStr(row.citation),
      committees: fromJson<CongressHearingCommittee[]>(row.committees),
      witnesses: fromJson<string[]>(row.witnesses),
      memberBioguideIds: fromJson<string[]>(row.member_bioguide_ids),
      detailUrl: str(row.detail_url),
      htmlUrl: nullableStr(row.html_url),
      pdfUrl: nullableStr(row.pdf_url),
      provenance: provFromRow(row),
    } as CongressHearing;
  },
};

export const fedCommunicationMapper: RowMapper<FedCommunication> = {
  toRow(r) {
    return {
      id: r.id,
      type: r.type,
  ...
  fromRow(row) {
    return {
      ...
      date: str(row.date),
      title: str(row.title),
      speaker: nullableStr(row.speaker),
      venue: nullableStr(row.venue),
      url: str(row.url),
...
export const ROW_MAPPERS: Record<DatasetId, RowMapper<never>> = {
  ...
  "congress-hearings": congressHearingMapper as RowMapper<never>,
  "fed-communications": fedCommunicationMapper as RowMapper<never>,
};
