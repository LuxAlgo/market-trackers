...
export class TrackerStore {
  ...
  /**
   * Which of `ids` already exist in a dataset's table — the discovery-time
   * diff a listing-walking source uses to fetch only documents it has never
   * ingested (see `sources/govinfo-hearings/source.ts`). Chunked by the
   * driver's parameter budget, so an arbitrarily long id list is safe.
   */
  async existingIds(datasetId: DatasetId, ids: readonly string[]): Promise<Set<string>> {
    const found = new Set<string>();
    if (ids.length === 0) return found;
    const dataset = datasetById(datasetId);
    const chunkSize = Math.max(1, this.driver.maxParams);
    for (let offset = 0; offset < ids.length; offset += chunkSize) {
      const chunk = ids.slice(offset, offset + chunkSize);
      const rows = await this.driver.all<{ id: string }>(
        `SELECT "id" FROM "${dataset.table}" WHERE "id" IN (${chunk.map(() => "?").join(", ")})`,
        [...chunk],
      );
