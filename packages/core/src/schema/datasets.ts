...
import { congressHearingSchema, type CongressHearing } from "./congress-hearing.js";
...
export interface DatasetDefinition<T = unknown> {
  ...
  /**
   * True for datasets whose source replaces the WHOLE dataset each sync
   * (e.g. quarterly bulk releases): "rows ingested on a day" is the entire
   * history there, so ingestion-day exports — daily deltas, latest.json,
   * feed.xml, per-entity feeds — are meaningless and enormous (a multi-GB
   * delta file per refresh). Snapshot shards + the manifest are the whole
   * export for these.
   */
...
export const DATASETS: {
  ...
  "congress-hearings": DatasetDefinition<CongressHearing>;
  "fed-communications": DatasetDefinition<FedCommunication>;
} = {
  "congress-trades": {
    id: "congress-trades",
  ...
  patents: {
    ...
    description:
      "US patents granted, from PatentsView bulk grant data on the USPTO Open Data Portal (free key), with best-effort assignee→ticker mapping.",
    schema: patentSchema,
    table: "patents",
    sources: ["patentsview"],
    exportDir: "patents/grants",
    // The ODP bulk product refreshes quarterly; allow a late release.
    freshnessWindowHours: 24 * 120,
    eventDate: (r) => r.grantDate,
    // Quarterly wholesale replacement: every refresh re-ingests 1976→today,
    // so a "daily delta" would be the entire multi-GB dataset.
    snapshotOnly: true,
  },
  "clinical-trials": {
    id: "clinical-trials",
  ...
  "congress-hearings": {
    ...
    description:
      "Published hearing transcript packages from GPO GovInfo's CHRG collection (keyless): title, committees, witnesses, member bioguide ids, and deep links to the transcript — an index with receipts; the text stays at govinfo.",
    schema: congressHearingSchema,
    table: "congress_hearings",
    sources: ["govinfo-hearings"],
    exportDir: "congress/hearings",
    // GPO publishes transcripts on its own schedule, often weeks after the
    // hearing; recess stretches with no new packages are normal.
    freshnessWindowHours: 24 * 30,
    eventDate: (r) => r.heldDate,
  },
  "fed-communications": {
    id: "fed-communications",
    title: "Federal Reserve communications",
    description:
      "Federal Reserve Board monetary-policy communications from the Board's public JSON feeds (keyless): FOMC statements and minutes announcements, speeches, and testimony — titles, speakers, dates, and deep links, verbatim.",
    schema: fedCommunicationSchema,
    table: "fed_communications",
    sources: ["federalreserve"],
    exportDir: "fed/communications",
    // Every sync re-upserts the whole feeds, so staleness means the
    // pipeline stopped running — judge it on a few days, not a Fed cycle.
    freshnessWindowHours: 96,
    eventDate: (r) => r.date,
  },
};
