...
describe("scripts/add-fixture.mjs --file", () => {
  ...
  it("refuses --url without MARKET_TRACKERS_CONTACT (fair access) and rejects bad parsers", () => {
    expect(() =>
      execFileSync(
        process.execPath,
        [SCRIPT, "--parser", "form-ownership", "--url", "https://www.sec.gov/Archives/x.txt"],
        { encoding: "utf8", env: { ...process.env, MARKET_TRACKERS_CONTACT: "" } },
      ),
    ).toThrow(/MARKET_TRACKERS_CONTACT/);
