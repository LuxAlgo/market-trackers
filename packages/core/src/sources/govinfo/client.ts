 ...
 * which hashes the raw row's field names, not this schema's). [verify-live]
 * confirmed field names: the file name lives in `justFileName` (falling
 * back to `name`, then the legacy/fixture-only `fileName`), and the
 * modification time in `formattedLastModifiedTime` (falling back to the
 * legacy/fixture-only `lastModified`). No field is required at the schema
 * level — a row missing all of a field's candidates is a per-row
 * extraction failure, not a validation failure, so one drifted field name
 * doesn't take the whole row down with it.
 */
export const govinfoListingFileSchema = z
  .object({
    folder: z.boolean().nullish(),
    justFileName: z.string().nullish(),
    name: z.string().nullish(),
    fileName: z.string().nullish(),
    lastModified: z.string().nullish(),
    formattedLastModifiedTime: z.string().nullish(),
  })
  .passthrough();
...
/**
 * A listing responded 200 with at least one non-folder file row, but every
 * row failed field extraction — the live shape has moved out from under
 * `govinfoListingFileSchema`'s field-name assumptions. Deliberately NOT an
 * `HttpError`: `syncBillType` downgrades only `HttpError` to a note, so
 * this must propagate past that and fail the sync loudly rather than
 * resolve as a quiet zero-row success.
 */
export class GovinfoListingDriftError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "GovinfoListingDriftError";
  }
}
...
/** Extracts the two fields this module actually needs from one validated row, or `null` if neither field candidate resolves to something usable. */
function extractListingEntry(row: GovinfoListingFile): ListingEntry | null {
  const fileName = row.justFileName ?? row.name ?? row.fileName ?? null;
  if (!fileName) return null;
  const lastModified = toIsoInstant(row.lastModified ?? row.formattedLastModifiedTime ?? null);
...
export async function fetchBillstatusListing(
  ...
  for (const raw of body.files) {
    ...
    sawNonFolderRow = true;
    const entry = extractListingEntry(parsed.data);
    if (entry) entries.push(entry);
  }

  // A 200 that named real (non-folder) files but yielded zero usable
  // entries is never a legitimate empty directory — a genuinely empty one
  // lists no non-folder rows at all. Reading it as empty here would hide a
  // whole listing's fields resolving to nothing extractable behind a
  // silent per-row `continue`, indistinguishable from real emptiness.
  if (sawNonFolderRow && entries.length === 0) {
    const fields = sampleRow ? Object.keys(sampleRow).sort().join(", ") : "(no rows)";
    throw new GovinfoListingDriftError(
      `${url}: ${body.files.length} file(s) listed but none survived field extraction — first row fields: [${fields}]`,
    );
  }
