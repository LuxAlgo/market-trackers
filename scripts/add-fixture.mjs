 ...
 *
 *   DOCKET_CONTACT=you@example.com node scripts/add-fixture.mjs \
 ...
 *   --url <edgar .txt url>              fetch from www.sec.gov (needs DOCKET_CONTACT)
...
if (args.url) {
  const contact = process.env.DOCKET_CONTACT;
  if (!contact) {
    fail(
      "DOCKET_CONTACT is not set. SEC fair access requires a declared contact in the " +
        "User-Agent; refusing to fetch anonymously. Example: DOCKET_CONTACT=you@example.com",
    );
  }
  ...
  const response = await fetch(txtUrl, {
    headers: {
      "user-agent": `docket-add-fixture/${core.DOCKET_VERSION} (${contact})`,
      accept: "text/plain, */*",
    },
  });
